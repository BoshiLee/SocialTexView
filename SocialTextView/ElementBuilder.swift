...
struct ElementBuilder {
    ...
    static func matches(from text: String, withSocialType type: SocialType) -> [String] {
        guard let regex = type.regularExpression else { return [] }
    ...
    static func matches(from text: String, withSocialType type: SocialType) -> [SocialElement] {
        ...
        return results.compactMap {
            ...
            if let subString = text.subString(with: nsRange) {
    ...
    static func matchesMentions(form text: String) -> [NSRange] {
        guard let regex = SocialType.mention.regularExpression else { return [] }
        let results = regex.matches(in: text, range: NSRange(text.startIndex..., in: text))
        return results.map { text.nsRange(from: Range($0.range, in: text)!) }
    }
    
    static func matchesFirstMention(form text: String) -> NSRange? {
        guard let regex = SocialType.mention.regularExpression else { return nil}
        let results = regex.matches(in: text, range: NSRange(text.startIndex..., in: text))
    ...
    static func replaceFirstMention(form text: inout String, with mentionDict: MentionDict) -> SocialElement? {
        ...
        guard let fristMentionTag = originText.subString(with: fristMention) else { return nil }
